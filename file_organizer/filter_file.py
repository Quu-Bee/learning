...
def filter_files(directory,all_extentions):
    ...
    filtered_files = [] # список, в котором будут храниться файлы с фильтруемыми расширениями
    files = [] # список, в котором будут храниться слова, разделённые знаком "."
    separated_names = list(map(lambda file: file.split("."), directory)) # список, в котором будут храниться все наименования в папке (directory), разделённые знаком "."
